...
package org.silvermob.sdk.renderingtestapp
...
import org.silvermob.sdk.renderingtestapp.plugplay.config.*
import org.silvermob.sdk.renderingtestapp.utils.BaseFragment
import org.silvermob.sdk.renderingtestapp.utils.ConfigurationViewSettings
import org.silvermob.sdk.renderingtestapp.utils.OpenRtbConfigs
import org.silvermob.sdk.renderingtestapp.widgets.EventCounterView
...
abstract class AdFragment : BaseFragment() {
    ...
    protected var refreshDelay = SilverMob.AUTO_REFRESH_DELAY_MIN / 1000
    ...
    override fun initUi(view: View, savedInstanceState: Bundle?) {
        arguments?.let {
            ...
            SilverMob.setStoredAuctionResponse(storedAuctionResponse)
    ...
    private fun setNoBidsAccountId(enable: Boolean) {
        if (enable) {
            SilverMob.setServerAccountId(getString(R.string.prebid_account_id_prod_no_bids))
        } else {
            SilverMob.setServerAccountId(getString(R.string.prebid_account_id_prod))
        }
    }
    ...
    protected fun configureOriginalPrebid() {
        val hostUrl = SilverMob.getPrebidServerHost().hostUrl
        val host = Host.CUSTOM
        host.hostUrl = hostUrl
        SilverMob.setServerAccountId(SilverMob.getPrebidServerAccountId())
        SilverMob.initializeSdk(requireContext(), null)
    }
