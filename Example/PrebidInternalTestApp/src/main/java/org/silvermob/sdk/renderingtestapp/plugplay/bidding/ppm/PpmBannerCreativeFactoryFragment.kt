...
class PpmBannerCreativeFactoryFragment : PpmBannerFragment() {
    ...
    override fun onAdLoaded(bannerView: BannerView?) {
        if (SilverMob.getCreativeFactoryTimeout() == DEFAULT_CREATIVE_FACTORY_TIMEOUT ||
            SilverMob.getCreativeFactoryTimeoutPreRenderContent() == DEFAULT_CREATIVE_FACTORY_TIMEOUT_PRERENDER_CONTENT) {
            events.failed(true)
        } else {
