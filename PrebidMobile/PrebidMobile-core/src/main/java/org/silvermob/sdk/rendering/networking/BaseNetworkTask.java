...
package org.silvermob.sdk.rendering.networking;
...
import org.silvermob.sdk.rendering.loading.FileDownloadTask;
import org.silvermob.sdk.rendering.networking.exception.BaseExceptionHolder;
...
public class BaseNetworkTask
    ...
    private URLConnection setHttpURLConnectionProperty(GetUrlParams param) throws Exception {
        ...
        connection.setConnectTimeout(SilverMob.getTimeoutMillis());
    ...
    private void setCustomHeadersIfAvailable(URLConnection connection) {
        if (!SilverMob.getCustomHeaders().isEmpty()) {
            for (Map.Entry<String, String> customHeader : SilverMob.getCustomHeaders().entrySet()) {
