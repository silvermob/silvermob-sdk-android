...
import org.silvermob.sdk.api.rendering.PrebidRenderer;
...
public class SdkInitializer {
    ...
    public static void init(
        ...
        if (SilverMob.isSdkInitialized() || InitializationNotifier.isInitializationInProgress()) {
            return;
        }
        ...
        if (SilverMob.logLevel != null) {
            LogUtil.setLogLevel(SilverMob.getLogLevel().getValue());
        }

        PrebidLogger customLogger = SilverMob.getCustomLogger();
        if (customLogger != null) {
            LogUtil.setLogger(customLogger);
        }

        try {
            SilverMob.registerPluginRenderer(new PrebidRenderer());
